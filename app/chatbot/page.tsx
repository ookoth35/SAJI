...
export default function ChatbotPage() {
  ...
  useEffect(() => {
    const initializeConversation = async () => {
      try {
        ...
        console.log("[v0] Conversation initialized:", id)
      } catch (error) {
  ...
  const escalateChat = async () => {
    try {
      console.log("[v0] Escalating chat:", conversationId)
  ...
  const handleSend = async () => {
    ...
    // If already escalated, send to agent (would be handled in real system)
    if (isEscalated) {
      // In a real system, this would send to the agent handler
      return
    }
    ...
    try {
      console.log("[v0] Sending message to AI:", { conversationId, message: userInput })
